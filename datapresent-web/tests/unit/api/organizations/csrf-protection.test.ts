...
describe("Organizations Route CSRF Protection (Fix #1)", () => {
  ...
  describe("PATCH — CSRF protection", () => {
    it("should return 403 when CSRF check fails on PATCH", async () => {
      // Arrange: auth passes, CSRF middleware returns a 403 response
      ...
      // Must not proceed to DB
      expect(mockPrisma.membership.findFirst).not.toHaveBeenCalled();
    });

    it("should call auth before withCsrfProtection on PATCH", async () => {
    ...
    it("should pass the request and userId to withCsrfProtection", async () => {
      ...
      // Assert: withCsrfProtection was called with the request and userId
  ...
  describe("DELETE — CSRF protection", () => {
    it("should return 403 when CSRF check fails on DELETE", async () => {
      // Arrange: auth passes, CSRF middleware returns a 403
      mockAuth.mockResolvedValue({
        user: { id: "user-123", email: "test@example.com" },
        expires: new Date(Date.now() + 86400).toISOString(),
      });
    ...
    it("should call auth before withCsrfProtection on DELETE", async () => {
      // Arrange
      mockWithCsrfProtection.mockResolvedValue(null);
      mockAuth.mockResolvedValue({
        user: { id: "user-123", email: "test@example.com" },
