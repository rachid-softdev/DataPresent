...
export function KpiGrid({ content }: KpiGridProps) {
  ...
  if (kpis.length === 0) {
    return (
      <div className="text-center text-muted-foreground py-8">Aucune donnée KPI disponible</div>
    );
  }
    ...
    <div className={`grid ${gridClass} gap-4`}>
      {kpis.map((kpi, i) => (
        <Card key={i} className="p-4">
          ...
          {kpi.sublabel && <p className="text-xs text-muted-foreground mt-1">{kpi.sublabel}</p>}
        </Card>
      ))}
    </div>
  );
