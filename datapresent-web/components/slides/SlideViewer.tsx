...
export function SlideViewer({ slides: initialSlides, reportId }: SlideViewerProps) {
  ...
  if (slides.length === 0) {
    return <div className="text-center py-12 text-muted-foreground">Aucune slide disponible</div>;
  }
      ...
      <div className="flex-1 min-w-0">
        ...
        {currentSlideCommentCount > 0 && (
          <div className="mt-4 flex items-center gap-2 text-sm text-muted-foreground">
            <MessageSquare className="w-4 h-4" />
            <span>
              {currentSlideCommentCount} commentaire{currentSlideCommentCount > 1 ? "s" : ""} sur
              cette slide
            </span>
            <button
