...
export const CommentThread = memo(function CommentThread({
  ...
  const generalComments = useMemo(() => comments.filter((c) => !c.slideId), [comments]);

  const CommentList = useMemo(
    () =>
      memo(function CommentList({ list, emptyMessage }: { list: Comment[]; emptyMessage: string }) {
        if (list.length === 0) {
            ...
            {currentSlideId && slideComments.length > 0 && (
              <div>
                ...
                <div className="space-y-1">
                  <CommentList
                    list={slideComments}
                    emptyMessage="Aucun commentaire sur cette slide"
                  />
                </div>
              </div>
            )}
