...
export async function getGenerateWorker(): Promise<Worker<unknown>> {
  ...
  workerInstance = new Worker<unknown>(
    ...
    async (job: Job) => {
      ...
      try {
        ...
        if (maxSlides !== null && requestedCount > maxSlides) {
          throw new Error(
            `Slide count ${requestedCount} exceeds plan limit of ${maxSlides} for organization ${orgId}`,
          );
        }
    ...
    },
    {
      ...
      retryStrategy: (job: Job) => {
        ...
        const delay =
          RETRY_BACKOFF[job.attemptsMade - 1] || RETRY_BACKOFF[RETRY_BACKOFF.length - 1];
        return delay;
      },
    },
  );
  ...
  workerInstance.on("failed", async (job: Job | undefined) => {
    ...
    if (reportId) {
      ...
      captureMessage(`Report generation failed permanently: ${reportId}`, "error", {
        jobId: job.id,
        reportId,
