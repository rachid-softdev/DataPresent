...
export async function POST(request: NextRequest) {
  const wrappedHandler = withFeature('EXPORT_PDF', async (req, ctx) => {
    // Your logic here
...
// With limit check
export async function POST(request: NextRequest) {
  const wrappedHandler = withLimit('EXPORT_PDF', async (req, ctx) => {
...
// With consume
export async function POST(request: NextRequest) {
  const wrappedHandler = withConsume('EXPORT_PDF', 1, async (req, ctx) => {
    // ctx.consumeResult has the result
    return NextResponse.json({ success: true })
  })
...
export async function POST(request: NextRequest) {
  ...
  if (request.headers.get('x-error')) {
