...
export async function sendMagicLinkEmail(to: string, magicLink: string) {
  ...
  const html = getEmailTemplate("magic-link", {
    ...
    infoLabel: "Sécurité",
    securityNote:
      "Ce lien est personnel et usage unique. Il expire dans 10 minutes. Si vous n'avez pas demandé ce lien, vous pouvez ignorer cet email.",
...
export async function sendWelcomeEmail(to: string, name: string) {
  ...
  const html = getEmailTemplate("welcome", {
    ...
    feature2Desc:
      "Notre IA transforme vos données en présentation professionnelle en quelques secondes.",
    feature3Icon: "📄",
