# QA end-to-end avec Playwright (DataPresent)

Runbook de test E2E manuel/automatisÃ© pour $WebDir (Next.js 16.2.7 (Turbopack), monorepo pnpm/Turborepo),
...
> Le serveur de dev dÃ©marre sur http://localhost:3000 (ou 3001/3002 si occupe).
...
Start-Process -FilePath cmd -ArgumentList "/c","pnpm web:dev" 
  -RedirectStandardOutput "$env:TEMP\qa\DataPresent.dev.txt" 
  -RedirectStandardError "$env:TEMP\qa\DataPresent.err.txt" -WindowStyle Hidden
...
- GET / â†’ **HTTP 200**.
GET / renvoie HTTP 200 avec le titre DataPresent. Racine publique, aucun mur d auth sur /.
...
Aucun bypass d auth dev trouve dans datapresent-web. Pages publiques testables ; pages protegees redirigent vers /login. Ajouter un bypass dev ou identifiants de seed pour les tester.
...
`
datapresent-web/scripts/qa\        # (Ã  crÃ©er si campagne complÃ¨te)
